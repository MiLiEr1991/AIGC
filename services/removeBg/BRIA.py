...
with open(os.path.join(BASE_DIR, './workflows/workflow_api_removebg_bria.json'), 'rb') as f:
    content = f.read()
...
def load_comfy_workflow(workflow_json, prompt_dict):
    ...
    prompt_update['5']['inputs']['image'] = prompt_dict.get("image_path")
...
def comfy_api(client_id, server_address, prompt):
    ...
    try:
        ...
        if images:
            res_dict['flag'] = 0
            res_dict['msg'] = "成功！"
            res_dict['image'] = images.get('6')[0]
            res_dict['mask'] = images.get('10')[0]
...
def remove_background_bria(Params):
    ...
    if res_comfy.get('flag') == 0:
        ...
        res_comfy['mask'] = base64.b64encode(res_comfy.get('mask')).decode()
...
if __name__ == "__main__":
    ...
    image_path = "./tmp/yaoming.jpeg"
    testParams = {
        "image": base64.b64encode(open(image_path, "rb").read()),
    }

    res = remove_background_bria(Params=testParams)
    if res.get("flag") != 0:
        print("faild!")
    else:
        image_base64 = base64.b64decode(res['image'])
        Image.open(io.BytesIO(image_base64)).show()
        mask_base64 = base64.b64decode(res['mask'])
        Image.open(io.BytesIO(mask_base64)).show()
